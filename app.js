...
//Ejercicio de repaso
class Cuentas {
    constructor(cuentas, saldos ) {
     this._cuentas = new Array[3]; //Array para almacenar cuentas de usuarios
     this._saldos = new Array[3]; //Array para almacenar los saldos de las cuentas
    } 
    //Métodos setters y getters
    setCuentas(index,cuenta){
     this._cuentas[index]=cuenta;
    }
    getCuentas(index){
     return this._cuentas[index];
    }
    setSaldos(index,saldo){
    ...
    crearCuenta(){
     for(i=0;i<this._cuentas.length; i++){
         ...
         this.setCuentas(i, cuenta);
         this.setSaldos(i, saldo);
     }
    }
    consultarSaldo(){
     let cuenta = prompt(`Ingrese el número de cuenta a consultar su saldo`);
     let index = this._cuentas.indexOf(cuenta); //Busca el indice del valor como argumento
     if(index!==1){
         alert(`El saldo de la cuenta: ${cuenta} es de: ${this.getSaldos(index)}`);
     }else {
         alert(`Cuenta no fué encontrada`);
     }
    }
 }
 ...
 let opcion;
 do{
     opcion = parseInt(prompt(`********** MENU BANCO BANCAYA **********
         \n 1 Crear Cuentas
